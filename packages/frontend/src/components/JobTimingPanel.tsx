...
type TimingJob = { _id: Id<"jobs">; status: string; startedAt?: number; completedAt?: number; cancelledAt?: number; timerStoppedAt?: number; currentPauseStartedAt?: number; pauseHistory?: PauseRecord[]; cleaners?: Array<{ _id: Id<"users">; name: string }>; assignedTeamMembers?: Array<{ _id: Id<"users">; name: string }>; assignedManagerName?: string | null };
...
export function JobTimingPanel({ job, userId, controls = false, ownerMode = false }: { job: TimingJob; userId?: Id<"users">; controls?: boolean; ownerMode?: boolean }) {
  ...
  useEffect(() => {
    if (!job.startedAt || job.completedAt || job.cancelledAt || job.timerStoppedAt) return;
    const interval = window.setInterval(() => setNow(Date.now()), 1000);
    return () => window.clearInterval(interval);
  }, [job.startedAt, job.completedAt, job.cancelledAt, job.timerStoppedAt]);
  ...
  const canControl = controls && !!userId && job.status === "in_progress" && !job.timerStoppedAt;
