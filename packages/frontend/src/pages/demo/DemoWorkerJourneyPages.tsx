...
export function DemoWorkerJobsPage({
  presentation,
  currentPath,
}: WorkerJourneyPageProps) {
  return (
    <DemoShell
  ...
  return (
    ...
    >
      {job ? (
        ...
        <ShowcaseUnknownJob presentation={presentation} />
      )}
    </DemoShell>
  );
}
    ...
    >
      {job ? (
        <div className="mx-auto max-w-4xl space-y-4">
          <Link
            href={buildShowcasePath("worker", `/jobs/${job.id}`, presentation)}
            className="inline-flex items-center gap-2 rounded-lg text-sm font-semibold text-primary-700 outline-none ring-primary-500 focus-visible:ring-2 focus-visible:ring-offset-4"
          >
          ...
          <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-800">
            <p className="font-semibold">Read-only Showcase workspace</p>
            <p className="mt-1">
              Checklist selections, photos, and submission controls are
