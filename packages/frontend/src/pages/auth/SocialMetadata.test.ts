...
describe("root social metadata", () => {
  it("provides one complete, crawler-visible social preview", () => {
    ...
    expect(html).toContain('<meta property="og:image" content="https://scrubscrubscrub.com/scrub-social-preview.png" />');
    ...
    expect(html).toContain('<meta name="twitter:image" content="https://scrubscrubscrub.com/scrub-social-preview.png" />');
  });

  it("does not use the removed homepage video poster for previews", () => {
    expect(html).not.toContain("scrub-demo-poster.png");
    expect(html).not.toMatch(/(?:og:image|twitter:image)[^>]+logo-full\.png/);
