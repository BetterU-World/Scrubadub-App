...
vi.mock("@/components/client/ClientPortalPage", () => ({
  ClientPortalSection: ({ title, children }: any) => createElement("section", null, createElement("h2", null, title), children),
  formatClientDate: (value: string) => value,
  formatClientMoney: (value: number) => `$${(value / 100).toFixed(2)}`,
}));
...
describe("client portal responsive contracts", () => {
  ...
  it("wraps long home content and exposes one reachable agreement and payment action", () => {
    ...
    expect(html).toContain('href="/client/service-agreements/agreement-1"');
    expect(html).toContain('href="/client/documents"');
    expect(html).toContain('href="/client/billing"');
  });
