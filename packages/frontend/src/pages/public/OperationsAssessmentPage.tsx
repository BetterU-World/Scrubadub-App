    ...
    <AssessmentShell>
      <main className="mx-auto max-w-5xl">
        <section className="mx-auto max-w-3xl py-4 text-center sm:py-10" aria-labelledby="assessment-intro-title">
          ...
          {progress.campaignId && <p className="mt-5 rounded-2xl border border-primary-200 bg-primary-50 p-4 text-primary-900">{t("assessment.giveaway.attribution")} <a href={`/giveaway?campaign=${encodeURIComponent(progress.campaignId)}#official-rules`} target="_blank" rel="noopener noreferrer" className="underline">{t("assessment.giveaway.rules")}</a> · <a href={`/giveaway?campaign=${encodeURIComponent(progress.campaignId)}#alternate-entry`} target="_blank" rel="noopener noreferrer" className="underline">{t("assessment.giveaway.alternate")}</a></p>}
