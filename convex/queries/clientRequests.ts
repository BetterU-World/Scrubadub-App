...
import { classifyRequestContext, isExistingClientServiceRequest } from "../lib/requestContext";
...
export const getCompanyRequests = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.status) {
      ...
      const classified = await Promise.all(requests.map(async request => ({ request, operational: await isExistingClientServiceRequest(ctx, request) })));
      return classified.filter(item => !item.operational).map(item => item.request);
    }
...
async function requireJobRequestStaff(ctx: any, args: { userId?: any; sessionToken: string }) {
...
function operationalStatus(request: any, job: any, today: string) {
  if (job?.status === "in_progress") return "in_progress";
  if (job && (job.status === "approved" || job.completedAt)) return "completed";
  if (job && !["cancelled", "approved"].includes(job.status)) return "scheduled";
...
async function projectJobRequest(ctx: any, request: any) {
  const [relationship, property, account, jobs] = await Promise.all([
    ctx.db.get(request.clientRelationshipId),
    request.propertyId ? ctx.db.get(request.propertyId) : null,
    request.commercialAccountId ? ctx.db.get(request.commercialAccountId) : null,
    ctx.db.query("jobs").withIndex("by_sourceClientRequestId", (q: any) => q.eq("sourceClientRequestId", request._id)).collect(),
  ]);
  const job = jobs.find((item: any) => item.status === "in_progress") ?? jobs.find((item: any) => !["cancelled", "approved"].includes(item.status)) ?? jobs.find((item: any) => item.status === "approved" || item.completedAt) ?? null;
  return {
    _id: request._id, source: request.source, requesterName: request.requesterName,
    relationshipName: relationship?.businessName || relationship?.displayName,
    locationName: property?.name || account?.clientName || request.propertySnapshot?.name,
    locationAddress: property?.address || account?.serviceAddress || request.propertySnapshot?.address,
    locationType: property ? "property" : "commercial_account",
    propertyId: request.propertyId, commercialAccountId: request.commercialAccountId,
    clientRelationship: relationship ? { _id: relationship._id, status: relationship.status, displayName: relationship.displayName, businessName: relationship.businessName } : null,
    requestedService: request.requestedService, requestedDate: request.requestedDate, timeWindow: request.timeWindow,
    requestedAddOns: request.requestedAddOnSnapshots ?? [], notes: request.notes, submittedAt: request.createdAt,
    status: operationalStatus(request, job, new Date().toISOString().slice(0, 10)),
    declinedAt: request.declinedAt, clientFacingDecisionNote: request.clientFacingDecisionNote,
    linkedJob: job ? { _id: job._id, status: job.status, scheduledDate: job.scheduledDate, startTime: job.startTime, durationMinutes: job.durationMinutes } : null,
    scheduledJob: job ? { _id: job._id, status: job.status, scheduledDate: job.scheduledDate, startTime: job.startTime, durationMinutes: job.durationMinutes } : null,
  };
}
...
export const listJobRequests = query({
  ...
  handler: async (ctx, args) => {
    ...
    const requests = await ctx.db.query("clientRequests").withIndex("by_companyId", q => q.eq("companyId", actor.companyId)).take(REQUEST_LIST_CAP);
    const eligible = (await Promise.all(requests.map(async request => ({ request, eligible: await isExistingClientServiceRequest(ctx, request) })))).filter(item => item.eligible);
    return Promise.all(eligible.map(item => projectJobRequest(ctx, item.request)));
  },
});

export const getJobRequestDetail = query({
  args: { userId: v.optional(v.id("users")), sessionToken: v.string(), requestId: v.id("clientRequests") },
  handler: async (ctx, args) => {
    const actor = await requireJobRequestStaff(ctx, args);
    const request = await ctx.db.get(args.requestId);
    if (!request || request.companyId !== actor.companyId) throw new Error("Access denied");
    if ((await classifyRequestContext(ctx, request)) !== "existing_client_service_request") return null;
...
export const listRequestsForPipeline = query({
  ...
  handler: async (ctx, args) => {
    ...
    const prospectRequests = (await Promise.all(requests.map(async request => ({ request, operational: await isExistingClientServiceRequest(ctx, request) })))).filter(item => !item.operational).map(item => item.request);
