...
describe("PR 6B ordinary operational data migration", () => {
  ...
  it("keeps shared operational reads while restricting requests and analytics to owners", async () => {
    ...
    await expect(t.query(api.queries.clientRequests.getCompanyRequests, common)).rejects.toThrow("canManageSalesAndCommercial permission required");
  ...
  it("keeps every approved public function session-verified while leaving public and internal entry points unchanged", () => {
    ...
    expect((source.match(/sessionToken:\s*v\.string\(\)/g) ?? [])).toHaveLength(55);
