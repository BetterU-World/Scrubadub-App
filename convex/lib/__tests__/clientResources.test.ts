...
beforeEach(() => { process.env.TOKEN_PEPPER = "e2-test-pepper"; process.env.APP_URL = "http://localhost:5173"; });
...
function read(t: ReturnType<typeof convexTest>, token: string, resourceId: string, download = false) {
  return t.fetch(`/client/resources/file?resourceId=${resourceId}${download ? "&download=1" : ""}`, { headers: { Origin: "http://localhost:5173", Authorization: `Bearer ${token}` } });
}
...
describe("Client Resources", () => {
  ...
  it("deduplicates retries and multiple relationships, then revokes only the selected relationship", async () => {
    ...
    expect((await read(s.t, s.tokens.client, s.guideId)).status).toBe(200);
    expect((await read(s.t, s.tokens.client, s.guideId, true)).headers.get("content-disposition")).toContain("attachment");
  ...
  it("retains assignments through replacement and blocks deletion until every access grant is removed", async () => {
    ...
    const newStorageId = await s.t.run((ctx) => ctx.storage.store(new Blob([replacement], { type: "application/pdf" })));
    ...
    const current = await read(s.t, s.tokens.client, s.guideId);
