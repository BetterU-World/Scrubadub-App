...
export const seed = internalMutation({ args: { passwordHashes: hashesValidator }, handler: async (ctx, args) => {
  ...
  const templateId = await ctx.db.insert("documentTemplates", { companyId, type: "service_agreement", name: "Bright Harbor recurring service agreement", body: "Fictional QA agreement for {{client_name}}.", isDefault: true, source: "scrub_editor", status: "active", version: 1, createdByUserId: ownerId, createdAt: now, updatedAt: now });
  ...
  await ctx.db.insert("invoices", { companyId, invoiceType: "commercial", clientRelationshipId: harborlightId, commercialAccountId: accountId, title: "Harborlight weekly turnovers", invoiceNumber: "QA-BH-1001", status: "issued", billingStartDate: isoDay(now, -14), billingEndDate: isoDay(now, -1), issueDate: isoDay(now, 0), dueDate: isoDay(now, 15), subtotalCents: 128000, baseSubtotalCents: 123500, addOnSubtotalCents: 4500, sourceProposalId: proposalId, taxCents: 0, totalCents: 128000, jobIds: [approvedJob, submittedJob], notes: "Inert QA invoice: no Stripe session or payment intent.", createdAt: now, updatedAt: now, issuedAt: now });
