...
const hashesValidator = v.object({ owner: v.string(), manager: v.string(), worker: v.string(), client: v.string() });
...
async function expectedPersonas(ctx: any, companyId?: any) {
  const staff: { owner: any; manager: any; worker: any } = { owner: null, manager: null, worker: null };
...
async function fixtureState(ctx: any) {
  ...
  const personas = await expectedPersonas(ctx, company?._id);
  if (!company) {
    if (personas.owner || personas.manager || personas.worker || personas.client) fail("reserved persona email exists without the fixture company");
...
export const status = internalQuery({ args: {}, handler: async (ctx) => {
  ...
  const state = await fixtureState(ctx);
  return state.company ? {
    exists: true, fixtureKey: QA_FIXTURE_KEY, companyId: state.company._id,
    personaIds: { owner: state.personas.owner._id, manager: state.personas.manager._id, worker: state.personas.worker._id, client: state.personas.client._id },
...
export const seed = internalMutation({ args: { passwordHashes: hashesValidator }, handler: async (ctx, args) => {
  ...
  if (existing.company) return {
    ...
    personaIds: { owner: existing.personas.owner._id, manager: existing.personas.manager._id, worker: existing.personas.worker._id, client: existing.personas.client._id },
  };
  ...
  return { created: true, fixtureKey: QA_FIXTURE_KEY, companyId, personaIds: { owner: ownerId, manager: managerId, worker: workerId, client: clientId }, summary: { properties: 4, jobs: 6, clientRelationships: 3, clientRequests: 2, proposals: 2 } };
} });
...
export const reset = internalMutation({ args: {}, handler: async (ctx) => {
  ...
  for (const user of [state.personas.owner, state.personas.manager, state.personas.worker]) {
  ...
  const authSessions = (await Promise.all([state.personas.owner, state.personas.manager, state.personas.worker].map((x) => indexed(ctx, "authSessions", "by_userId", "userId", x._id)))).flat();
