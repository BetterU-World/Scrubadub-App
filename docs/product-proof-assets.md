...
The available in-app capture surface produced lossless PNG sources at the viewport's native pixel density rather than an emulated 2× device scale. The PNG sources remain local capture intermediates and are not served or committed. Production images are displayed below their intrinsic dimensions where practical and were visually checked for legibility.

## Served assets

| Files (AVIF + WebP fallback) | Source route | Capture viewport | Output dimensions | Focal crop and purpose |
| --- | --- | ---: | ---: | --- |
| `product-proof-owner-dashboard-1200.*` | `/internal/demo/owner?presentation=1` | 1440×900 | 1200×860 | Primary hero view: onboarding, operational counts, upcoming jobs, and red flags |
| `product-proof-owner-dashboard-mobile-415.*` | `/internal/demo/owner?presentation=1` | 430×932 | 415×899 | Purpose-captured narrow Owner Dashboard view used by the hero below 640px |
| `product-proof-owner-operations-1200.*` | `/internal/demo/owner?presentation=1` | 1440×900 | 1200×500 | Owner-section crop focused on operational counts, jobs, and red flags |
...
6. Export AVIF first and WebP fallback. Keep the hero desktop under 180 KB, hero mobile under 100 KB, and every other asset under 150 KB.
