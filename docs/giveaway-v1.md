...
## Activation configuration and remaining release steps

1. The campaign configuration is armed with `rulesApproved: true` and `enabled: true`, while the existing server-time gate keeps entries closed until September 21, 2026 at 12:00 AM Eastern. The close remains October 30, 2026 at 11:59 PM Eastern; the drawing target remains on or about November 2, 2026. Merge and the approved release process are still required before the public configuration can take effect.
2. Retain the final owner/business review record with the frozen rules version. Sponsor is confirmed as **Scrubadub Solutions LLC, operator of SCRUB**; no d/b/a or separate SCRUB entity should be asserted without verified support. No attorney, government, or regulatory approval is represented by this configuration.
...
5. Verify `VITE_ENABLE_OPERATIONS_ASSESSMENT=true` for the primary Assessment route and deploy matching backend/frontend configuration through the approved release process. AMOE depends on campaign gates, not the Assessment feature flag. This PR does not deploy either service.
...
This PR does not deploy production, perform a drawing, or send email. The campaign becomes publicly available only after the approved release process deploys this configuration and the server-time opening threshold is reached.
...
- At the PR #200 validation point, `enabled: false` and `rulesApproved: false` were reconfirmed. This activation PR changes those configuration gates to `true`; no Convex deployment, manual production deployment, winner drawing, or email sending occurred.
